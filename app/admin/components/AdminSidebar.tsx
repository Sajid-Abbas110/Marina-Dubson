...
export default function AdminSidebar({ isCollapsed, toggleCollapse, isOpen, setIsOpen }: AdminSidebarProps) {
    ...
    return (
        <>
            ...
            <button
                type="button"
                className="lg:hidden fixed top-4 left-4 z-50 p-2.5 rounded-2xl text-gray-900 bg-white dark:bg-[#00120d] border border-gray-100 dark:border-white/10 shadow-2xl active:scale-90 transition-all font-black"
                onClick={() => setIsOpen(!isOpen)}
            >
                {isOpen ? <X className="h-6 w-6 text-gray-900 dark:text-white" /> : <Menu className="h-6 w-6 text-gray-900 dark:text-white" />}
            </button>

            {/* Elite Sidebar for desktop */}
            <aside
                className={`fixed inset-y-0 left-0 z-[120] flex flex-col border-r border-gray-200 dark:border-white/5 transition-all duration-300 ease-in-out transform lg:translate-x-0 
                    ${theme === 'dark' ? 'bg-[#001712]' : 'bg-[#eef5f3]'}
            ...
            >
                ...
                <div className={`flex flex-col relative overflow-hidden h-[100px] justify-center border-b border-gray-100 dark:border-white/5 transition-all duration-300 ${isCollapsed ? 'px-0' : 'px-6'}`}>
                    ...
                    <div className="absolute top-0 left-0 w-full h-full bg-gradient-to-tr from-primary/5 dark:from-primary/10 via-transparent to-indigo-600/5 dark:to-indigo-600/10 animate-pulse"></div>

                    <div className={`flex items-center gap-4 relative z-10 ${isCollapsed ? 'justify-center' : ''}`}>
                        <div className="relative group">
                            <div className="absolute -inset-1.5 bg-gradient-to-r from-primary to-indigo-600 rounded-xl blur-lg opacity-10 dark:opacity-40 group-hover:opacity-100 transition duration-500 animate-pulse"></div>
                            <div className="relative h-10 w-10 rounded-xl bg-gradient-to-br from-primary to-indigo-800 flex items-center justify-center text-white font-black text-lg shadow-2xl border border-white/20 transform group-hover:rotate-12 transition-transform duration-500">
                                MD
                            </div>
                        </div>
                ...
                <div className={`p-6 border-t border-gray-100 dark:border-white/5 ${isCollapsed ? 'items-center px-2' : ''}`}>
                    {!isCollapsed ? (
                        ...
                        <div className="h-8 w-8 mx-auto rounded-full bg-indigo-500/10 flex items-center justify-center border border-indigo-500/20">
