...
export default function TacticalCommMatrix() {
    ...
    useEffect(() => {
        fetchConversations()
        // Get current user from token if possible
        const token = localStorage.getItem('token')
        if (token) {
            try {
    ...
    useEffect(() => {
        if (activeChat) {
            ...
            const interval = setInterval(() => fetchMessages(activeChat), 5000) // Poll every 5s
    ...
    return (
        <div className="h-full w-full flex flex-col xl:flex-row gap-4 bg-white dark:bg-[#0f172a]/20 p-4 lg:p-6 font-poppins transition-colors duration-500 overflow-hidden">
            {/* Sidebar Context */}
            <div className="xl:w-[320px] w-full flex flex-col gap-4 flex-shrink-0 animate-in fade-in slide-in-from-left-8 duration-700">
                <div className="glass-panel p-5 space-y-4 rounded-[2rem]">
                    <div className="flex items-center justify-between">
                        <div>
                            <h1 className="text-xl font-black text-gray-900 dark:text-white tracking-tighter uppercase leading-none">
                                Comm <span className="text-primary italic">Matrix</span>
                            </h1>
                            <p className="text-[8px] font-black text-gray-400 dark:text-slate-500 uppercase tracking-[0.4em] mt-1.5">Network Active</p>
                        </div>
                        <div className="h-9 w-9 rounded-xl bg-primary/10 dark:bg-primary/20 border border-primary/20 dark:border-primary/40 flex items-center justify-center group cursor-pointer hover:bg-primary transition-all">
                            <Zap className="h-3.5 w-3.5 text-primary dark:text-primary group-hover:text-white" />
                        </div>
                    </div>
                ...
                <div className="flex-1 glass-panel rounded-[2rem] flex flex-col overflow-hidden min-h-[300px]">
                    <div className="px-6 py-4 border-b border-gray-100 dark:border-white/5 flex items-center justify-between bg-gray-50/50 dark:bg-white/[0.02]">
                        <span className="text-[9px] font-black tracking-[0.3em] text-gray-400 dark:text-slate-500 uppercase">Linked Channels</span>
                        <Filter className="h-3.5 w-3.5 text-gray-400 hover:text-primary cursor-pointer transition-colors" />
                    </div>
                    ...
                    <div className="flex-1 overflow-y-auto p-2 space-y-1 custom-scrollbar">
                        {loading ? (
                            ...
                            <button
                                ...
                                className={`w-full p-4 rounded-2xl text-left transition-all relative group flex items-start gap-4 ${activeChat === c.id ? 'bg-primary text-white shadow-lg shadow-primary/20' : 'bg-transparent hover:bg-emerald-50 dark:hover:bg-white/[0.04]'}`}
                            >
                                <div className="relative flex-shrink-0">
                                    <div className={`h-10 w-10 rounded-xl bg-gray-100 dark:bg-white/10 flex items-center justify-center text-sm font-black ${activeChat === c.id ? 'text-white' : 'text-primary'} shadow-md`}>
                                        {c.name.split(' ').map((n: any) => n[0]).join('')}
                                    </div>
                                    <div className="absolute -bottom-0.5 -right-0.5 h-3.5 w-3.5 rounded-full border-2 border-white dark:border-[#1e293b] bg-emerald-500"></div>
                                </div>
                                <div className="flex-1 min-w-0 pt-0.5">
                                    <div className="flex justify-between items-center mb-0.5">
                                        <h4 className={`text-[10px] font-black uppercase tracking-tight truncate ${activeChat === c.id ? 'text-white' : 'text-gray-900 dark:text-white'}`}>{c.name}</h4>
                                        <span className={`text-[7px] font-bold ${activeChat === c.id ? 'text-white/60' : 'text-gray-400'}`}>
                                            {c.time ? new Date(c.time).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : ''}
                                        </span>
                                    </div>
                                    <p className={`text-[9px] font-medium truncate opacity-60 ${activeChat === c.id ? 'text-white' : 'text-gray-500 dark:text-slate-400'}`}>{c.lastMsg}</p>
                                </div>
                            </button>
                        ))}
                    </div>
            ...
            {/* Tactical Stream */}
            <div className="flex-1 glass-panel rounded-[2.5rem] flex flex-col overflow-hidden relative animate-in fade-in slide-in-from-right-8 duration-700 shadow-2xl shadow-primary/5">
                ...
                <div className="px-6 py-4 border-b border-gray-100 dark:border-white/5 bg-white/50 dark:bg-white/[0.02] flex items-center justify-between backdrop-blur-3xl">
                        ...
                        {activeConv ? (
                            <>
                                <div className="relative">
                                    ...
                                    <div className="absolute -bottom-0.5 -right-0.5 h-3 w-3 bg-emerald-500 border-2 border-white dark:border-[#0f172a] rounded-full"></div>
                                </div>
                                <div>
                    ...
                    <div className="flex items-center gap-2">
                        <button className="p-2 rounded-xl hover:bg-gray-100 dark:hover:bg-white/10 text-gray-400 transition-all"><Camera className="h-4 w-4" /></button>
                        <button className="p-2 rounded-xl hover:bg-gray-100 dark:hover:bg-white/10 text-gray-400 transition-all"><Mic className="h-4 w-4" /></button>
                        <button className="p-2 rounded-xl hover:bg-gray-100 dark:hover:bg-white/10 text-gray-400 transition-all"><MoreHorizontal className="h-4 w-4" /></button>
                    </div>
                </div>

                {/* Message Stream */}
                <div ref={scrollRef} className="flex-1 overflow-y-auto p-6 lg:p-10 space-y-8 dark:bg-gradient-to-b dark:from-[#020617]/50 dark:to-transparent custom-scrollbar">
                    <div className="flex flex-col items-center py-4">
                        <div className="px-6 py-2 rounded-xl bg-gray-50 dark:bg-white/5 border border-gray-100 dark:border-white/10 flex items-center gap-3 text-[9px] font-black text-gray-400 dark:text-slate-400 uppercase tracking-widest">
                ...
                {/* Tactical Input */}
                <div className="p-6 bg-white/80 dark:bg-[#020617]/80 border-t border-gray-100 dark:border-white/5 backdrop-blur-3xl">
                    <form onSubmit={sendMessage} className="flex items-center gap-4 bg-gray-50 dark:bg-[#0f172a] rounded-[2rem] p-3 border border-gray-200 dark:border-white/5 shadow-inner">
                        <button type="button" className="h-10 w-10 rounded-xl bg-white dark:bg-white/5 text-gray-400 hover:text-primary transition-all flex items-center justify-center flex-shrink-0">
                        ...
                        />
                        <div className="flex items-center gap-2">
                            <button type="button" className="h-10 w-10 rounded-xl bg-white dark:bg-white/5 text-gray-400 hover:text-yellow-500 transition-all flex items-center justify-center">
                                <Smile className="h-5 w-5" />
                            </button>
                            <button
                                type="submit"
                                className="h-12 px-6 rounded-2xl bg-primary text-white font-black text-[9px] uppercase tracking-widest flex items-center gap-3 hover:bg-emerald-600 transition-all shadow-lg shadow-primary/20"
                            >
    ...
    return (
        <div className={`flex flex-col ${isMe ? 'items-end' : 'items-start'} animate-in fade-in slide-in-from-${isMe ? 'right' : 'left'}-4 duration-500`}>
            <div className="flex items-center gap-3 mb-2 px-2">
                {!isMe && <span className="text-[9px] font-black text-gray-900 dark:text-white uppercase tracking-tighter">{name}</span>}
                <span className="text-[7px] font-black text-gray-400 dark:text-slate-500 uppercase tracking-widest">{time}</span>
                {isMe && <span className="text-[9px] font-black text-primary uppercase tracking-tighter">Command Unit</span>}
            </div>
            <div className={`max-w-[85%] p-5 rounded-[2rem] shadow-lg relative border ${isMe ? 'bg-primary border-primary/20 text-white rounded-tr-none' : 'bg-white dark:bg-white/5 border-gray-100 dark:border-white/5 text-gray-700 dark:text-slate-200 rounded-tl-none shadow-sm'}`}>
