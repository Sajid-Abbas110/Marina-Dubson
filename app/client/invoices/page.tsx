                        ...
                        {loading ? (
                            ...
                            >
                                <div className="flex flex-col sm:flex-row sm:items-center gap-6 sm:gap-10">
                                    <div className={`h-16 w-16 rounded-2xl flex items-center justify-center text-2xl shadow-sm flex-shrink-0 ${inv.status === 'PAID' ? 'bg-emerald-50 text-emerald-500' : 'bg-rose-50 text-rose-500'}`}>
                                        <DollarSign className="h-8 w-8" />
                                    </div>
                                    <div className="space-y-1">
                                        <div className="flex flex-wrap items-center gap-3">
                                            <span className="text-[10px] font-black text-primary uppercase tracking-widest">{inv.invoiceNumber}</span>
                                            <h4 className="text-lg sm:text-xl font-black text-gray-900 uppercase tracking-tight">{inv.booking.proceedingType}</h4>
                                        </div>
                                ...
                                <div className="flex flex-wrap items-center justify-between sm:justify-end gap-6 sm:gap-16 pt-6 sm:pt-0 border-t border-gray-50 sm:border-t-0 mt-6 sm:mt-0">
                                    <div className="text-left sm:text-right">
                                        <p className="text-[9px] font-black text-gray-400 uppercase tracking-widest mb-1">Amount Due</p>
                                        <p className="text-xl sm:text-2xl font-black text-gray-900 tracking-tighter">${inv.total.toFixed(2)}</p>
                                    </div>

                                    <div className="flex items-center gap-4">
                                        <div className={`px-4 sm:px-6 py-2 sm:py-2.5 rounded-2xl text-[9px] sm:text-[10px] font-black uppercase tracking-widest border ${inv.status === 'PAID' ? 'bg-emerald-50 text-emerald-600 border-emerald-100' : 'bg-rose-50 text-rose-600 border-rose-100'
                                            }`}>
                                            {inv.status}
                                        </div>

                                        <button className="h-12 w-12 sm:h-14 sm:w-14 rounded-xl sm:rounded-[1.5rem] bg-white border border-gray-100 flex items-center justify-center text-gray-300 hover:text-primary hover:border-primary/20 hover:shadow-xl transition-all">
