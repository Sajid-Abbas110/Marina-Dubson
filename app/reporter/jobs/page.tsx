        ...
        <div className="min-h-screen bg-slate-50 text-slate-900 font-poppins selection:bg-blue-100 selection:text-blue-900">
            ...
            <main className="max-w-7xl mx-auto px-6 sm:px-12 py-12 relative z-10">
                ...
                <div className="mb-12">
                    <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-blue-100 border border-blue-200 text-blue-700 text-[10px] font-bold uppercase tracking-wider mb-4">
                        <Briefcase className="h-3 w-3" /> Opportunities Found: {jobs.length}
                    </div>
                    <h2 className="text-4xl sm:text-5xl font-extrabold text-slate-900 tracking-tight leading-tight mb-8">
                        View Available <span className="text-blue-600">Assigments</span>
                    </h2>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-4 gap-10">
                    ...
                    <aside className="lg:col-span-1 space-y-8">
                        <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
                            ...
                            <div className="flex flex-col gap-2">
                                <SidebarFilter active={filter === 'ALL'} onClick={() => setFilter('ALL')} label="All Jobs" icon={<Globe className="h-4 w-4" />} count={jobs.length} />
                                <SidebarFilter active={filter === 'REMOTE'} onClick={() => setFilter('REMOTE')} label="Remote" icon={<Zap className="h-4 w-4" />} count={jobs.filter(j => j.appearanceType === 'REMOTE').length} />
                                <SidebarFilter active={filter === 'LOCAL'} onClick={() => setFilter('LOCAL')} label="In-Person" icon={<MapPin className="h-4 w-4" />} count={jobs.filter(j => j.appearanceType === 'IN_PERSON').length} />
                            </div>
                        </div>
                    ...
                    <div className="lg:col-span-3 space-y-4">
                        {loading ? (
                            ...
                            <div key={job.id} className="bg-white border border-slate-200 rounded-2xl p-6 sm:p-8 hover:shadow-md hover:border-blue-200 transition-all flex flex-col sm:row items-center justify-between gap-6 shadow-sm group">
                                <div className="flex items-center gap-6 w-full">
                                    ...
                                    <button
                                        ...
                                        className="h-14 px-6 rounded-xl bg-blue-600 text-white font-bold text-xs uppercase tracking-widest hover:bg-blue-700 transition-all shadow-md active:scale-95 flex items-center gap-2 group-hover:shadow-blue-200"
                                    >
                                        Place Bid <Zap className="h-4 w-4" />
                                    </button>
                                </div>
                            </div>
                        ))}
            ...
            {showBidModal && (
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
                    <div className="absolute inset-0 bg-slate-900/40 backdrop-blur-sm" onClick={() => setShowBidModal(false)}></div>
                    <div className="relative w-full max-w-xl bg-white rounded-[2rem] p-8 sm:p-12 shadow-2xl border border-slate-200 overflow-hidden animate-in zoom-in-95 duration-200">
                        <div className="flex items-center gap-4 mb-8">
                            <div className="h-12 w-12 rounded-xl bg-blue-600 flex items-center justify-center text-white">
                                <Zap className="h-6 w-6" />
                            </div>
                            <div>
                                <h2 className="text-xl font-bold text-slate-900 uppercase">Submit Bid</h2>
                                <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mt-1">Assignment: {selectedJob?.bookingNumber}</p>
                            </div>
                        </div>
                        ...
                        <form onSubmit={handleBidSubmit} className="space-y-6">
                            <div className="grid grid-cols-2 gap-6">
                                <div className="space-y-1.5">
                                    <label className="text-[10px] font-bold text-slate-500 uppercase tracking-widest ml-1">Bid Amount ($)</label>
                                    <input
                                        required
                                        type="number"
                                        className="w-full h-14 bg-slate-50 border-2 border-slate-200 rounded-xl px-5 text-sm font-bold text-slate-900 focus:bg-white focus:border-blue-600 outline-none transition-all"
                                        placeholder="0.00"
                                ...
                                <div className="space-y-1.5">
                                    ...
                                    <select
                                        className="w-full h-14 bg-slate-50 border-2 border-slate-200 rounded-xl px-5 text-sm font-bold text-slate-900 focus:bg-white focus:border-blue-600 outline-none transition-all cursor-pointer"
                                        value={bidData.timeline}
                                        onChange={(e) => setBidData({ ...bidData, timeline: e.target.value })}
                                    >
                                        <option value="Standard">Standard (10 Days)</option>
                                        <option value="Expedited">Expedited (3 Days)</option>
                                        <option value="Elite">Realtime / Daily</option>
                                    </select>
                                </div>
                            </div>

                            <div className="space-y-1.5">
                                <label className="text-[10px] font-bold text-slate-500 uppercase tracking-widest ml-1">Notes / Special Proposals</label>
                                <textarea
                                    className="w-full min-h-[140px] bg-slate-50 border-2 border-slate-200 rounded-xl p-5 text-sm font-medium text-slate-900 focus:bg-white focus:border-blue-600 outline-none transition-all resize-none"
                                    placeholder="Briefly describe your availability or strategy..."
                                    value={bidData.notes}
                            ...
                            <div className="flex gap-4 pt-4">
                                <button
                                    ...
                                    className="flex-1 h-14 rounded-xl bg-slate-100 text-[10px] font-bold uppercase text-slate-500 hover:bg-slate-200 transition-all"
                                >
                                    Cancel
                                </button>
                                <button
                                    type="submit"
                                    className="flex-1 h-14 bg-blue-600 text-white rounded-xl font-bold text-[10px] uppercase tracking-widest shadow-lg hover:bg-blue-700 transition-all active:scale-95"
                                >
                                    Submit Official Bid
                                </button>
                            </div>
