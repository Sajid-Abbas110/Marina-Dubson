...
import {
    Search, Send, Shield, Globe, Video, Mic, MoreHorizontal,
...
// Camera Modal removed in favor of quick Video Link generation
...
export default function TacticalCommMatrix() {
    ...
    // Auto-load ALL users into linked channels on mount (Admin only)
    useEffect(() => {
    ...
    const handleSearch = async (q: string) => {
        ...
        if (currentUser?.role !== 'ADMIN') return; // Only Admins can mass search right now
    ...
    const handleScanNodes = async () => {
        if (currentUser?.role !== 'ADMIN') {
            alert('Contacts scanner is restricted to Admin. Other roles see active assignments only.');
            return;
        }
        setIsScanning(true)
        try {
    ...
    const insertVideoLink = () => {
        const link = `[LINK:https://meet.jit.si/MD_SecureCall_${Math.random().toString(36).substring(7)}|Join Secure Video Call]`;
        setNewMessage(p => p + (p ? ' ' : '') + link);
                                ...
                                {conversations.length === 0 && (
                                    <div className="p-4 space-y-3">
                                        {currentUser?.role === 'ADMIN' ? (
                                            ...
                                            <div className="py-8 text-center text-muted-foreground font-black text-[10px] uppercase opacity-50">No active channels yet...</div>
                                        )}
                                    </div>
                                )}
                                {conversations.map(c => (
            ...
            <div className={`flex-1 glass-panel rounded-[2.5rem] flex-col overflow-hidden relative animate-in fade-in slide-in-from-right-8 duration-700 shadow-2xl shadow-primary/5 border border-border bg-card ${activeChat ? 'flex h-full' : 'hidden xl:flex'}`}>
                ...
                <div className="px-6 py-4 border-b border-border bg-background/50 flex items-center justify-between backdrop-blur-3xl flex-shrink-0">
                    ...
                    <div className="flex items-center gap-1">
                        {/* Video Call */}
                        <button onClick={insertVideoLink} title="Generate Video Link" className="p-2 rounded-xl hover:bg-primary/10 text-muted-foreground hover:text-primary transition-all"><Video className="h-4 w-4" /></button>
                        {/* Mic / Voice */}
                        {isRecording
